...
class ProjectSeven extends Component {
  render() {
    const mainText = (
      "Mixtape is an app for sharing music and is built with React and the are.na API. Using are.na as a back-end users can upload mp3s from around the internet to make mixes of songs that might not be available on larger streaming services. The name of the are.na channel can be added to the end of the app's url and the songs from that channel will be loaded into page dynamically."
    );
